...
def engaged(game, clock, a, b, ring="ring_gold"):
    ...
    assert cmd(game, b, "accept")["text"].startswith(("You say yes", "Kamu menjawab ya"))
    clock.advance(2)
...
def test_a_proposal_needs_a_ring_a_room_and_a_yes(make_game, clock):
    ...
    assert budi.sent[-1]["text"] == "Ani menyodorkan cincin perak dan memintamu menikah dengannya. Ucapkan terima atau tolak."
    assert ceri.sent[-1]["text"] == "Ani offers Budi a ring. The room holds its breath."
    text(game, budi, "tolak")
    ...
    clock.advance(601)
    text(game, ani, "lamar Budi")
    yes = text(game, budi, "terima")
    assert yes["text"].startswith("Kamu menjawab ya, dan Ani memasangkan cincin perak di jarimu.")
...
@pytest.mark.parametrize("words, venue, tier, style, when", [
    ...
    ("paviliun megah netral 14:00", "pavilion", "grand", "neutral", (2026, 9, 25, 14, 0)),
    ("aula kubah bintang mewah cahaya bintang besok 09:30", "star_hall", "luxurious", "starlight",
     (2026, 9, 26, 9, 30)),
    ...
    ("balairung sederhana netral", "grove_hall", "simple", "neutral", None),
    ("pavilion 14:00", "pavilion", None, None, (2026, 9, 25, 14, 0)),
])
def test_bookings_are_read_in_both_languages(make_game, words, venue, tier, style, when):
...
def test_booking_a_hall(make_game, clock):
    ...
    booked_reply = text(game, ani, "pesan pernikahan paviliun megah netral 14:00")
    ...
    assert text(game, dani, "jadwal pernikahan")["text"] == \
        "Pernikahan yang akan datang: Ani dan Budi, Paviliun Melati, Jumat 25-09-2026 pukul 14:00 waktu stasiun; " \
        "Ceri dan Dani, Paviliun Melati, Jumat 25-09-2026 pukul 15:00 waktu stasiun."
...
def test_invitations_and_answers(make_game, clock):
    ...
    assert ceri.sent[-1]["text"].startswith("Kamu diundang ke pernikahan Ani dan Budi: Paviliun Melati")
    assert text(game, ani, "undang Ceri ke pernikahan")["text"] == "Ceri is invited already."
    assert text(game, ani, "invite Budi to the wedding")["text"] == "The couple doesn't need an invitation."
    dani = join(game, "Dani")
    cmd(game, dani, "bye")
    text(game, budi, "undang Dani ke pernikahan")
    dani = join(game, "Dani")
    assert "You're invited to the wedding of Ani and Budi" in dani.sent[1]["text"]
    assert text(game, ceri, "hadir")["text"].startswith("Kamu akan hadir di pernikahan Ani dan Budi")
    ...
    assert text(game, ceri, "undangan")["text"].startswith("Undanganmu: Ani dan Budi, Paviliun Melati")
...
def test_cancelling_gives_money_back_when_early_enough(make_game, clock):
    ...
    assert text(game, budi, "batalkan pernikahan")["text"] == "Your wedding is cancelled. 4000 credits come back."
    ...
    credits = ani.session.char["credits"]
    assert text(game, rafli, "batalkan pernikahan Budi")["text"] == \
...
def test_ending_an_engagement_cancels_the_wedding(make_game, clock):
    ...
    text(game, budi, "akhiri kemitraan")
    assert "a wedding you booked is cancelled" in budi.sent[-1]["text"]
    credits = ani.session.char["credits"]
    text(game, budi, "konfirmasi akhiri")
...
def test_the_starlight_rite_step_by_step(make_game, clock):
    ...
    assert "Ibu Sekar says: Ani, Budi: each of you, light a lantern for your promise. Say light lantern." in ceri.texts()
    text(game, ani, "light lantern")
    assert ceri.texts()[-1] == "Ani lights a lantern, and a small warm light rises in the hall."
    text(game, budi, "nyalakan lentera")
    tick_until(game, clock, clock() + 5)
    assert "Ibu Sekar says: Now bring your two lights together, into one. Say join the lights." in ceri.texts()
    ...
    text(game, budi, "ikrar Aku akan selalu penasaran bersamamu.")
    ...
    tick_until(game, clock, clock() + 6)
    assert "Ibu Sekar says: Two lights, one path. Ani, Budi: you walk the Way together now. Congratulations!" in \
        ceri.texts()
    ...
    memory = text(game, ceri, "baca kenangan")["text"]
...
def test_the_neutral_ceremony_asks_each_for_a_yes(make_game, clock):
    ...
    assert "Bu Safira says: Welcome, everyone, to the wedding of Ani and Budi. Thank you for being here." in \
        ceri.texts()
    tick_until(game, clock, clock() + 8)
    text(game, ani, "vow Always.")
    text(game, budi, "vow Always, too.")
    tick_until(game, clock, clock() + 5)
    assert "Bu Safira says: Ani, do you choose to share your life with Budi? Say yes, or no." in ceri.texts()
    ...
    assert "Bu Safira says: Budi, do you choose to share your life with Ani? Say yes, or no." in ceri.texts()
    text(game, budi, "yes")
    tick_until(game, clock, clock() + 5)
    assert "Bu Safira says: Then please sign the station's register, both of you. Say sign." in ceri.texts()
    text(game, ani, "sign")
    text(game, budi, "tanda tangan")
    tick_until(game, clock, clock() + 5)
    assert "Bu Safira says: With your own words and your own names, Ani and Budi, you are married. Congratulations!" \
        in ceri.texts()
