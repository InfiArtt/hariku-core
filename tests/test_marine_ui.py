...
# Opens the Sea Conditions windows with real wxPython in a separate process
# (conftest.py mocks wx here), through the real extension loader and hotkeys.
# Open-Meteo is stubbed there, so nothing leaves the machine.
...
def test_sea_conditions_windows(tmp_path):
    env = dict(os.environ, APPDATA=str(tmp_path), PYTHONIOENCODING="utf-8")
    result = subprocess.run(
        [sys.executable, os.path.join(ROOT, "tests", "_marine_ui_check.py")],
        capture_output=True, text=True, encoding="utf-8", errors="replace",
        timeout=200, env=env,
    )
    output = result.stdout + result.stderr
    for stage in ("OK main_window", "OK load", "OK no_location_hint", "OK weather_default_inland",
