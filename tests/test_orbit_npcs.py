...
# both languages, schedules and walking the map on a fixed clock, the night
...
def test_every_resident_speaks_both_languages_and_can_walk_their_day(world):
    ...
    for nid, d in npcs.items():
        assert 1 <= d["voice"] <= 10 and d["topics"] and d["greet"]["en"] and d["unknown"]["id"]
...
def test_a_broken_resident_is_refused(world):
    for change in (lambda n: n["npcs"]["jali"].update(voice=12),
                   lambda n: n["npcs"]["jali"]["greet"]["id"].pop(),
                   lambda n: n["npcs"]["jali"]["topics"]["coffee"]["say"].update(id=["{nope}"]),
...
def test_residents_are_named_apart_from_players(make_game):
    game = make_game()
    tono = join(game, "Tono")
    assert "Residents here: Kapten Bayu, the ferry's pilot." in tono.sent[1]["text"]
    assert "Nobody else is here" not in tono.sent[1]["text"]
    who = cmd(game, tono, "who")
    assert who["text"].startswith("1 online:") and "Bayu" not in who["text"]
    look = cmd(game, tono, "look", a="Bayu")["text"]
    assert look.startswith("Kapten Bayu, the ferry's pilot.") and "not a player" in look
    assert cmd(game, tono, "whisper", to="Bayu", a="hi")["text"] == \
        "Kapten Bayu is a resident, not a player: talk to Kapten Bayu where you find them."
    ...
    assert refused.sent[0]["code"] == "name_reserved"


def test_the_residents_list_says_where_everyone_is(make_game):
    game = make_game()
    tono = join(game, "Tono", lang="id")
    reply = text(game, tono, "penduduk")["text"]
    assert reply.startswith("Para penduduk, yang bukan pemain: Bang Jali, bartender Kantin, di Kantin;")
    assert "Kelana, pengelana yang datang hanya di malam hari, sedang tidak bertugas" in reply
...
def test_talking_greets_you_in_the_residents_voice_and_lists_the_topics(make_game):
    ...
    budi = join(game, "Budi")
    tono.clear()
    cmd(game, tono, "talk", to="Bayu")
    greeting, topics = tono.sent[-2], tono.sent[-1]
    assert greeting["k"] == "say" and greeting["actor"] == "Kapten Bayu" and greeting["voice"] == 5
    assert greeting["text"] == f"Kapten Bayu says: {greeting['words']}"
    assert topics["text"].startswith("You can ask Kapten Bayu about: ferry, worlds, customs, kancil and favour.")
    assert "Say ask Bayu about, and a topic." in topics["text"]
    assert budi.sent[-1]["text"] == "Tono chats with Kapten Bayu."


def test_asking_about_topics_in_either_language(make_game):
    game = make_game()
    tono = join(game, "Tono", lang="id")
    reply = text(game, tono, "tanya Bayu tentang feri")
    assert reply["actor"] == "Kapten Bayu" and reply["words"].startswith("Starling berangkat 2 menit lagi.")
    assert text(game, tono, "tanya Kapten Bayu soal pabean")["words"].startswith("Pabean paling sering")
    assert text(game, tono, "tanya Bayu customs")["words"].startswith("Pabean")        # English words work too
    unknown = text(game, tono, "tanya Bayu tentang resep rendang")
    assert unknown["words"] in ("Aku tahu rute, bukan teka-teki. Tanya soal feri atau dunia-dunia.",
                                "Tak bisa kujawab. Tapi aku bisa mengantarmu ke tempat yang tahu.")
...
def test_talking_by_the_old_ways_of_saying_it(make_game):
    ...
    tono = join(game, "Tono", lang="id")
    tono.clear()
    cmd(game, tono, "say", a="dengan Bayu")                 # "bicara dengan Bayu" from any client
    assert tono.sent[-2]["actor"] == "Kapten Bayu" and tono.sent[-1]["text"].startswith("Kamu bisa bertanya")
    tono.clear()
    cmd(game, tono, "say", a="dengan senang hati")          # just words
    assert tono.sent[-1]["k"] == "said"
    assert text(game, tono, "halo Bayu")["actor"] == "Kapten Bayu"
    assert tono.sent[-2]["text"] == "Kamu menyapa Kapten Bayu."
...
def test_nobody_to_talk_to_or_somebody_elsewhere(make_game, clock):
    ...
    assert cmd(game, tono, "talk", to="Jali")["text"] == "Bang Jali isn't here. You might find Bang Jali in the Cantina."
    assert cmd(game, tono, "talk", to="Kelana")["text"] == "Kelana is off duty now. Try again later."
    budi = join(game, "Budi")
...
def test_live_answers(make_game, clock):
    ...
    assert cmd(game, tono, "ask", to="Jali", a="who")["words"] == \
        "3 people are about on the station right now, Ani and Budi among them."
    game.start_event("double_xp")
    assert "for another 60 minutes" in cmd(game, tono, "ask", to="Jali", a="events")["words"]
    game.market.overrides["coffee"] = {"price": 30.0, "until": clock() + 600}
    market = cmd(game, tono, "ask", to="Jali", a="market")["words"]
    ...
    assert "Tono has more credits" in cmd(game, tono, "ask", to="Jali", a="gossip")["words"]
    place(game, tono, "engineering")
    assert cmd(game, tono, "ask", to="Harsa", a="my progress")["words"] == \
        "You're level 1, a trainee pilot. 60 more XP and you'll be level 2. Keep your hands busy."
    place(game, tono, "gear_shop")
    special = cmd(game, tono, "ask", to="Tegar", a="special")["words"]
    ...
    assert cmd(game, tono, "ask", to="Nilam", a="my pet")["words"].startswith("You don't have a pet yet!")
    place(game, tono, "dock")
    assert "The Starling leaves in" in cmd(game, tono, "ask", to="Bayu", a="ferry")["words"]
...
def test_the_night_traveller(make_game, clock):
    ...
    assert cmd(game, tono, "ask", to="Kelana", a="the time")["words"].startswith(
        "It's 19:00 by the station's clock, at night.")
    assert cmd(game, tono, "ask", to="Kelana", a="the hunt")["words"].startswith("No song is being sought")
    at(clock, 5, 0, day=26)
    for _ in range(3):
        clock.advance(15)
        game.tick()
...
def test_a_resident_walks_to_the_next_place_and_both_rooms_hear_it(make_game, clock):
    ...
    left = [m for m in watcher.events("leave") if m.get("actor") == "Pak Gino"]
    assert left and left[0]["dir"] == path[0][0] and left[0]["text"].startswith("Pak Gino heads")
    budi = join(game, "Budi")
    ...
    assert any(m.get("actor") == "Pak Gino" for m in budi.events("leave"))
...
def test_talking_every_day_and_new_topics_bring_you_closer(make_game, clock):
    ...
    cmd(game, tono, "ask", to="Bayu", a="ferry")
    cmd(game, tono, "ask", to="Bayu", a="ferry")
    cmd(game, tono, "ask", to="Bayu", a="customs")
    assert memory(game, tono, "bayu")["affinity"] == 3
    clock.advance(86400)
    cmd(game, tono, "greet", to="Bayu")
    assert memory(game, tono, "bayu")["affinity"] == 4
    clock.advance(86400)
    tono.clear()
    cmd(game, tono, "emote", e="wave", to="Bayu")
    assert memory(game, tono, "bayu")["affinity"] == 5
    assert "Kapten Bayu now counts you as an acquaintance." in tono.texts("info")
    assert [m for m in tono.sent if m.get("sound") == "npc_warm"]
    assert "Kapten Bayu knows you as an acquaintance." in cmd(game, tono, "look", a="Bayu")["text"]
    assert "Residents who know you: Kapten Bayu (an acquaintance)." in cmd(game, tono, "profile")["text"]
...
def test_gestures_at_a_resident_are_answered(make_game):
    ...
    budi = join(game, "Budi")
    tono.clear()
    cmd(game, tono, "emote", e="hug", to="Bayu")
    assert tono.texts("emote")[-2:] == ["You hug Kapten Bayu.", "Kapten Bayu returns the hug warmly."]
    assert budi.texts("emote")[-2:] == ["Tono hugs Kapten Bayu.", "Kapten Bayu returns Tono's hug warmly."]
    assert tono.sent[-1]["emote"] == "hug" and tono.sent[-1]["actor"] == "Kapten Bayu"
...
def test_gifts_once_a_day_and_more_for_what_they_like(make_game, clock):
    ...
    cmd(game, tono, "give", to="Bayu", n=1, item="coffee")
    assert tono.sent[-2]["text"] == "You give 1 sack of coffee to Kapten Bayu."
    ...
    clock.advance(2)
    cmd(game, tono, "give", to="Bayu", n=1, item="iron")
    assert tono.sent[-1]["words"].startswith("Another one?") and memory(game, tono, "bayu")["affinity"] == 4
    clock.advance(86400)
    cmd(game, tono, "give", to="Kapten", n=1, item="Bayu coffee")        # "beri Kapten Bayu kopi"
    assert said_by(tono, "Kapten Bayu")[-1]["words"].startswith("Oh! This is exactly")
    assert tono.sent[-1]["text"] == "Kapten Bayu now counts you as an acquaintance."
    assert cmd(game, tono, "give", to="Bayu", n=1, item="compass")["text"] == \
        "Kapten Bayu can't accept compasses: those stay with you."
    assert cmd(game, tono, "give", to="Bayu", n=50, item="credits")["words"].startswith("Keep your credits")
...
def test_a_favour_asked_brought_and_rewarded(make_game, clock):
    ...
    char = tono.session.char
    place(game, tono, "engineering")
    cmd(game, tono, "ask", to="Harsa", a="work")
    assert tono.sent[-2]["words"].startswith("The pump housings need patching.")
    assert tono.sent[-1]["text"] == "Pak Harsa is waiting for 3 pieces of scrap metal. When you have them, " \
                                    "give them to Pak Harsa."
    char["inventory"]["scrap"] = 2
    assert cmd(game, tono, "give", to="Harsa", n=2, item="scrap")["text"] == \
        "Pak Harsa needs 3 pieces of scrap metal, all at once."
    char["inventory"]["scrap"] = 4
    credits, xp = char["credits"], char["xp"]
    cmd(game, tono, "give", to="Harsa", n=3, item="scrap")
...
def test_topics_and_errands_that_need_a_friend(make_game, clock):
    ...
    tono = join(game, "Tono")
    place(game, tono, "cantina")
    assert cmd(game, tono, "ask", to="Jali", a="secret")["words"] == \
        "Ask me that again when we know each other a little better, Tono."
    talk = [m["text"] for m in (cmd(game, tono, "talk", to="Jali"),)][0]
    assert "Get to know Bang Jali better, and there will be more to talk about." in talk
    game.store.save_npc_memory(dict(memory(game, tono, "jali"), affinity=15))
    assert cmd(game, tono, "ask", to="Jali", a="secret")["words"].startswith("Alright, for you.")
    cmd(game, tono, "ask", to="Jali", a="favour")
    assert tono.sent[-2]["words"].startswith("My sambal jar is empty")
...
def test_a_shopkeeper_gives_friends_a_small_discount(make_game):
    ...
    assert listing.endswith("Mas Tegar gives you a friend's price: 5 percent off.")
...
def test_idle_lines_are_rare_and_never_while_players_talk(make_game, clock):
    ...
    for _ in range(40):
        ...
        game.tick()
    assert not said_by(tono, "Bang Jali")
...
def test_idle_lines_fit_the_room(make_game, clock):
    ...
    for _ in range(120):
        clock.advance(15)
        game.tick()
    gino = [m["text"] for m in said_by(tono, "Pak Gino")]
    assert gino and all("martabak" not in line and "tower" not in line for line in gino)
