...
# Opens Preferences and uses the Hariku Voice page with real wxPython, in a
# separate process (conftest.py mocks wx here). The voices there are fakes:
# nothing is spoken or played.
...
def test_hariku_voice_page(tmp_path):
    env = dict(os.environ, APPDATA=str(tmp_path), PYTHONIOENCODING="utf-8")
    result = subprocess.run(
        [sys.executable, os.path.join(ROOT, "tests", "_voice_ui_check.py")],
