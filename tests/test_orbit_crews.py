...
def found(game, clock, name="Bintang", captain="Ani"):
...
def test_founding_a_crew(make_game, clock):
    ...
    assert cmd(game, ani, "crew_create", a="Bintang")["text"] == "You can found a crew from level 3."
    ready(game, ani, credits=100)
    assert cmd(game, ani, "crew_create", a="Bintang")["text"] == \
    ...
    made = cmd(game, ani, "crew_create", a="  Bintang   Timur ")
    assert made["text"].startswith("You found the crew Bintang Timur, with yourself as its captain (500 credits;")
    ...
    assert (crew["name"], crew["name_key"], role) == ("Bintang Timur", "bintangtimur", "captain")
    assert cmd(game, ani, "crew_create", a="Other")["text"] == "You're already in the crew Bintang Timur."
    info = cmd(game, ani, "crew")["text"]
    assert info == "The crew Bintang Timur: captain Ani; 1 members: Ani (online). 0 points."
    assert "crewmate" in set(game.store.achievements_of(char["id"]))
    # the name is taken, whatever its spaces and case
    budi = join(game, "Budi")
    ready(game, budi)
    assert cmd(game, budi, "crew_create", a="bintang timur")["text"] == "There's already a crew called bintang timur."
    assert cmd(game, budi, "crew_create", a="BintangTimur")["text"] == "There's already a crew called BintangTimur."
...
def test_invitations_are_answered_like_offers(make_game, clock):
    ...
    assert cmd(game, ani, "crew_invite", to="Budi")["text"] == "You invite Budi to the crew Bintang."
    ask = budi.sent[-1]
    assert ask["k"] == "offer" and ask["ask"] == "crew" and ask["actor"] == "Ani"
    assert ask["text"] == "Ani invites you to join the crew Bintang. Type accept or decline."
    assert cmd(game, ani, "crew_invite", to="Budi")["text"] == "Budi hasn't answered your invitation yet."
    joined = cmd(game, budi, "accept")
    assert joined["text"].startswith("Welcome aboard: you're in the crew Bintang.")
    assert any(m.get("text") == "Budi joins the crew Bintang!" for m in ani.sent)
    assert game.store.crew_of(budi.session.char["id"])[1] == "member"
    # declining
    cmd(game, ani, "crew_invite", to="Cici")
    assert cmd(game, cici, "decline")["text"] == "You decline the invitation to Bintang."
    assert ani.sent[-1]["text"] == "Cici declines your invitation."
    # the natural sentence, from either client
    assert text(game, ani, "undang Cici ke kru")["text"] == "You invite Cici to the crew Bintang."
    clock.advance(121)
    game.tick()
    assert cici.sent[-1]["text"] == "The invitation to Bintang has run out."
...
def test_the_newest_ask_is_the_one_answered(make_game, clock):
    ...
    assert cmd(game, budi, "decline")["text"] == "You decline the invitation to Bintang."
...
def test_crew_chat_reaches_the_crew_anywhere_and_no_one_else(make_game, clock):
    ...
    sent = cmd(game, budi, "crew_say", a="  halo   semua ")
    assert sent["k"] == "crew_sent" and sent["text"] == "You tell the crew Bintang: halo semua"
    assert sent["words"] == "halo semua" and sent["voice"] == 4 and sent["brief"] == "Told the crew."
    heard = ani.sent[-1]
    assert heard["k"] == "crew" and heard["actor"] == "Budi" and heard["sound"] == "crew_chat"
    assert heard["text"] == "Budi, to the crew Bintang: halo semua" and heard["words"] == "halo semua"
    assert not any(m.get("k") == "crew" for m in cici.sent)
    assert text(game, ani, "kru bilang siap!")["text"] == "You tell the crew Bintang: siap!"
    assert budi.sent[-1]["text"] == "Ani, to the crew Bintang: siap!"
    assert cmd(game, cici, "crew_say", a="hai")["text"] == "You're not in a crew."
    # "tell crew ...": both clients send it as a whisper to "crew"
    cmd(game, ani, "whisper", to="crew", a="lewat bisikan")
    assert budi.sent[-1]["text"] == "Ani, to the crew Bintang: lewat bisikan"
    assert cmd(game, cici, "whisper", to="kru", a="hai")["text"] == "Nobody called kru is on the station."
...
def test_leaving_passes_the_captaincy_on_and_the_last_one_out_ends_it(make_game, clock):
    ...
    assert cmd(game, ani, "crew_leave")["text"] == "You leave the crew Bintang."
    assert any(m.get("text") == "Budi is now the captain of the crew Bintang." for m in cici.sent)
    assert game.store.crew_of(budi.session.char["id"])[1] == "captain"
    assert cmd(game, cici, "crew_leave")["text"] == "You leave the crew Bintang."
    assert cmd(game, budi, "crew_leave")["text"] == "You leave the crew Bintang."
    assert game.store.crew_by_key("bintang") is None
    ready(game, ani)
    assert cmd(game, ani, "crew_create", a="Bintang")["text"].startswith("You found the crew Bintang")
...
def test_the_captain_kicks_hands_over_and_sets_the_motto(make_game, clock):
    ...
    assert cmd(game, ani, "crew_kick", to="Dedi")["text"] == "Dedi isn't in the crew Bintang."
    ...
    assert cici.sent[-1]["text"] == "Ani has taken you off the crew Bintang."
    assert game.store.crew_of(cici.session.char["id"]) is None
    assert cmd(game, ani, "crew_motto", a="Ke bintang!")["text"] == "The crew's motto is now: Ke bintang!"
    assert cmd(game, budi, "crew")["text"].endswith("Motto: Ke bintang!")
    cmd(game, ani, "crew_captain", to="Budi")
    assert budi.sent[-1]["text"] == "Budi is now the captain of the crew Bintang."
    ...
    assert "Captain of the crew Bintang." in cmd(game, cici, "look", a="Budi")["text"]
    assert "Crew: Bintang." in cmd(game, cici, "profile", to="Ani")["text"]
...
def test_members_xp_are_the_crews_points_and_the_board(make_game, clock):
    ...
    assert game.store.crew_by_key("bintang")["points"] == 42
    board = cmd(game, budi, "crews")["text"]
    assert board == ("The crews with the most points: 1. Comet, 50 points, 1 members; 2. Bintang, 42 points, "
                     "2 members. Yours, Bintang: 42 points.")
    assert text(game, budi, "papan kru")["text"] == board
    dedi = join(game, "Dedi")
...
def test_admins_can_disband_a_crew(make_game, clock):
    ...
    invite_in(game, clock, ani, budi)
    rafli = join(game, "Rafli")
    assert cmd(game, ani, "admin", op="crew_disband", a="Bintang")["text"] == "Only the station's admins can do that."
    assert cmd(game, rafli, "admin", op="crew_disband", a="Nope")["text"] == "There's no crew called Nope."
    assert text(game, rafli, "bubarkan kru Bintang")["text"] == "The crew Bintang is disbanded."
    assert budi.sent[-1]["text"] == "The station's admins have disbanded the crew Bintang."
    assert game.store.crew_of(ani.session.char["id"]) is None and game.store.crew_by_key("bintang") is None
...
def test_a_version_6_database_gets_the_crew_tables(tmp_path, clock, monkeypatch):
    ...
    crew_id = store.create_crew("Bintang", "bintang", quila["id"], 1.0)
