...
status line, toggling without the focus moving, Apply), the game window
("Open Orbit": the Messages list and the Command field with their labels
first, Enter sends, another player's words arrive without the focus or the
selection moving, Up brings back the last command, Escape hides it), and
Aruna ("orbit who", "orbit say ..."), with what is said landing in Last
result. Finally unloading.
...
services.play = lambda name: sounds.append(name)
...
assert kinds == ["StaticText", "TextCtrl", "StaticText", "TextCtrl", "StaticText", "Choice",
                 "StaticText", "CheckBox", "CheckBox", "CheckBox", "StaticText", "Slider",
                 "CheckBox", "Button", "StaticText", "TextCtrl", "StaticText", "StaticText"], kinds
...
assert page.btn_connect.GetLabel() == "&Disconnect"
...
assert kinds == ["StaticText", "ListBox", "StaticText", "TextCtrl", "Button", "Button",
...
window.txt_command.SetValue("go to the cantina")
fire(window.txt_command, wx.EVT_TEXT_ENTER)
assert window.txt_command.GetValue() == ""
assert pump(lambda: any("You walk through the Promenade to the Cantina." in window.lst_messages.GetString(i)
...
sari.send_json({"t": "hello", "v": 1, "lang": "en", "name": "Sari", "job": "engineer",
                "secret": "5a" * 32})
sari.send_json({"t": "cmd", "c": "go", "a": "cantina"})
